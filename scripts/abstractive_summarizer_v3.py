...
def _embedding_from_bert():
  ...
  with tf.device("/device:CPU:0"):
      vocab_of_BERT = hub.KerasLayer(BERT_MODEL_URL, trainable=False, name='BERT_Layer')
  ...
  return (embedding_matrix, vocab_of_BERT, dec_model, enc_model)
...
class AbstractiveSummarization(tf.keras.Model):
    ...
    def __init__(self, num_layers, d_model, num_heads, dff, vocab_size, output_seq_len, rate=0.1):
        ...
        #BertLayer(d_embedding=d_model, trainable=False)       
        embedding_matrix, self.vocab_of_BERT, dec_model, enc_model = _embedding_from_bert()
        self.bert = enc_model.predict
        self.dec_bert = dec_model.predict
    ...
    def refine_summary(self, 
        ...
        # (batch_size x (seq_len - 1), seq_len, d_bert)
        context_vectors = self.dec_bert((dec_inp_ids, dec_inp_mask, dec_inp_segment_ids))
