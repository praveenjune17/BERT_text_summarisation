...
from metrics import optimizer, loss_function, label_smoothing, get_loss_and_accuracy, tf_write_summary, monitor_run, convert_wordpiece_to_words
...
for (step, (input_ids, target_ids_)) in enumerate(train_dataset):
  if step+1 > config.start_from_batch:
    count+=1
    start=time.time()
    ...
    if eval_frequency == 0:
      ...
      (val_acc, rouge_score, bert_score) = calc_validation_loss(
                                                                val_dataset, 
                                                                step+1, 
                                                                val_step, 
