...
class Generator(tf.keras.Model):
  ...
  def call(self, dec_output, final_output, attention_weights, encoder_input, 
    ...
    i1, i2 = tf.meshgrid(tf.range(batch),[0], indexing="ij")    #eval working
    i1 = tf.tile(i1[:, :, tf.newaxis], [1, inp_shape, tar_shape])
    i2 = tf.tile(i2[:, :, tf.newaxis], [1, inp_shape, tar_shape])
