...
def beam_search(symbols_to_logits_fn,
  ...
  #log_probs = tf.tile(initial_log_probs, [batch_size, eos_id[0][0]-1])
  # Expand each batch and state to beam_size
  alive_seq = _expand_to_beam_size(initial_ids, beam_size)
  alive_seq = tf.expand_dims(alive_seq, axis=2)  # (batch_size, beam_size, 1)
  log_probs = tf.tile(alive_seq, [1, 1, decode_length + 1])
  log_probs = tf.expand_dims(log_probs, axis=3)
  log_probs = tf.tile(log_probs, [1, 1, 1, eos_id[0][0]-1])
  if use_tpu:
  ...
  def grow_topk(i, alive_seq, alive_log_probs, states):
    ...
    return topk_seq, topk_log_probs, topk_scores, topk_finished, states, logits

  def inner_loop(i, alive_seq, alive_log_probs, finished_seq, finished_scores,
                 finished_flags, states, log_probs):
    ...
    topk_seq, topk_log_probs, topk_scores, topk_finished, states, log_probs = grow_topk(
  ...
  def _is_not_finished(i, unused_alive_seq, alive_log_probs,
                       ...
                       unused_finished_in_finished, unused_states, log_probs):
  ...
  if use_tpu:
    ...
    finished_flags, states, log_probs) = tf.nest.map_structure(
                                tf.stop_gradient, 
                                tf.while_loop(
                                   cond=_is_not_finished,
                                   body=inner_loop, loop_vars=[
                                       tf.constant(0), alive_seq, alive_log_probs, finished_seq,
                                       finished_scores, finished_flags, states, log_probs
                                   ],
                                   shape_invariants=[
                                       tf.TensorShape([]),
                                       inner_shape,
  ...
  try:
    predictions = tf.transpose(tf.convert_to_tensor(log_probs_list), perm=(1,2,0,3))
  except:
    print(log_probs_list)
  #print(log_probs_list)
  return finished_seq, finished_scores, predictions
